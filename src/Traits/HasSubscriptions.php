...
use AbnDevs\Subscriptions\Exceptions\DuplicateException;
use AbnDevs\Subscriptions\Exceptions\InvalidPlanSubscription;
...
use AbnDevs\Subscriptions\Services\SubscriptionPeriod;
use Carbon\Carbon;
...
trait HasSubscriptions
{
    ...
    {
        return $this->morphMany(config('laravel-subscriptions.models.plan_subscription'), 'subscriber', 'subscriber_type', 'subscriber_id');
    }
    ...
    public function subscription(?string $subscriptionTag = null)
    {
        ...
        $subscriptionTag = $subscriptionTag ?? config('laravel-subscriptions.main_subscription_tag');
     ...
     *
     * @return \AbnDevs\Subscriptions\Models\PlanSubscription|null
     */
    public function subscribedPlans()
    {
        $planIds = $this->subscriptions->reject->isInactive()->pluck('plan_id')->unique();

        return app(config('laravel-subscriptions.models.plan'))->whereIn('id', $planIds)->get();
    }
     ...
     * @param \AbnDevs\Subscriptions\Models\Plan|\AbnDevs\Subscriptions\Models\PlanCombination $planCombination Plan pricing and invoice data
    ...
    {
        $tag = $tag ?? config('laravel-subscriptions.main_subscription_tag');
