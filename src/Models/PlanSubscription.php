...
use AbnDevs\Subscriptions\Exceptions\UsageDenied;
use AbnDevs\Subscriptions\Services\Period;
use AbnDevs\Subscriptions\Traits\BelongsToPlan;
use AbnDevs\Subscriptions\Traits\HasFeatures;
use AbnDevs\Subscriptions\Traits\HasGracePeriod;
use AbnDevs\Subscriptions\Traits\HasGracePeriodUsage;
use AbnDevs\Subscriptions\Traits\HasPricing;
use AbnDevs\Subscriptions\Traits\HasSubscriptionPeriodUsage;
use AbnDevs\Subscriptions\Traits\HasTrialPeriodUsage;
use AbnDevs\Subscriptions\Traits\HasSchedules;
use Carbon\Carbon;
use DB;
...
class PlanSubscription extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('laravel-subscriptions.tables.plan_subscriptions'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                ...
                'alpha_dash',
                'max:150',
                Rule::unique(config('laravel-subscriptions.tables.plan_subscriptions'))->where(function ($query) {
    ...
    {
        return $this->hasMany(config('laravel-subscriptions.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('laravel-subscriptions.models.plan_subscription_usage'),
            config('laravel-subscriptions.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('laravel-subscriptions.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('laravel-subscriptions.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('laravel-subscriptions.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \AbnDevs\Subscriptions\Models\PlanSubscriptionUsage|null
     */
